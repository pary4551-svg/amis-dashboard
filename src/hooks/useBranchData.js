...
export function useKpiForRange(rows, year, startMonth, endMonth) {
  return useMemo(() => {
    ...
    return {
      ...
      외형매출:        ext,
      외형매출_prev:   prevRows.length > 0 ? pExt : null,
      외형매출_yoy:    pctYoY(ext, pExt),
      판관비:          pg,
      판관비_prev:     prevRows.length > 0 ? pPg : null,
      판관비_yoy:      pctYoY(pg, pPg),
      판관비율:        pgRatio,
      판관비율_prev:   pgRatioPrev,
      판관비율_yoy:    ppYoY(pgRatio, pgRatioPrev),
      영업이익:        op,
      영업이익_prev:   prevRows.length > 0 ? pOp : null,
      영업이익_yoy:    pctYoY(op, pOp),
      영업이익률:      opRatio,
      영업이익률_prev: opRatioPrev,
      영업이익률_yoy:  ppYoY(opRatio, opRatioPrev),
      당기순이익:      ni,
      당기순이익_prev: prevRows.length > 0 ? pNi : null,
